...
def valor_diaria_financeiro_registro(
    ...
    """
    Valor-base enviado ao Financeiro.

    Regra atual: esse valor é exatamente o que o supervisor/engenheiro
    informou no apontamento em ``custo_pago``. O custo cadastrado/importado
    do colaborador NÃO altera este valor; ele é usado somente pela
    Controladoria/custos de serviço.
    """
    registro = registro or {}
...
def valor_extra_registro(registro):
    """Campo legado mantido no banco por compatibilidade; não compõe mais o apontamento."""
    return 0.0
...
TRELLO_SYNC_TTL_SEGUNDOS = 300
TRELLO_AUTO_SYNC_INTERVALO_SEGUNDOS = 12 * 60 * 60
TRELLO_AUTO_SYNC_RETRY_SEGUNDOS = 30 * 60
...
def _garantir_schema_trello_obras():
    """
    Guarda a identidade real do card do Trello na obra.

    Antes a sincronização considerava uma obra existente apenas pelo NOME.
    Isso podia esconder cards diferentes com nomes iguais/parecidos e também
    não atualizava a unidade quando um card mudava no Trello.
    """
    if DB_BACKEND != "NEON" or not hasattr(supabase, "_connect"):
        return False
...
def _registrar_estado_sync_trello(
    *,
    ...
    try:
        _garantir_tabela_snapshot_trello()
        resultado = {
            "sucesso": bool(sucesso),
            "total_cards": int(total_cards or 0),
            "inseridas": int(inseridas or 0),
            "atualizadas": int(atualizadas or 0),
            "existentes": int(existentes or 0),
            "erro": str(erro or "")[:500],
        }

        with supabase._connect() as conn:
            with conn.cursor() as cur:
                # Garante linha 1 mesmo se ainda não houver snapshot válido.
                cur.execute(
                    """
                ...
                if sucesso:
                    cur.execute(
                        """
                        UPDATE trello_snapshot
                           SET sincronizado_obras_em = NOW(),
                               ultima_tentativa_obras_em = NOW(),
                               ultimo_resultado_obras = %s::jsonb
                         WHERE snapshot_id = 1
                        """,
                    ...
                    cur.execute(
                        """
                        UPDATE trello_snapshot
                           SET ultima_tentativa_obras_em = NOW(),
                               ultimo_resultado_obras = %s::jsonb
                         WHERE snapshot_id = 1
                        """,
                        (json.dumps(resultado, ensure_ascii=False),),
...
def _estado_sync_trello_obras():
    ...
    try:
        ...
        if isinstance(row, dict):
            return (
                row.get("sincronizado_obras_em"),
                row.get("ultima_tentativa_obras_em"),
            )
...
def executar_sincronizacao_trello(id_lista_target=None, id_card_target=None, listas_precarregadas=None, cards_precarregados=None, forcar_atualizacao=False):
    ...
    try:
        ...
        # O JSON público do quadro contém os cards da lista inteira.
        # A persistência abaixo usa o ID real do Trello, e não somente o nome,
        # para garantir que todos os cards de EM EXECUÇÃO possam existir no banco.
        total_cards_execucao = len(cards_execucao)
        ...
        try:
            if DB_BACKEND == "NEON" and hasattr(supabase, "_connect"):
                with supabase._connect() as conn:
                    with conn.cursor() as cur:
                        ...
                        for obra in obras_atuais:
                            nome_norm = normalizar(obra.get("nome", ""))
                            unid_norm = normalizar(obra.get("unidade", ""))
                            if nome_norm:
                                por_nome_unidade.setdefault(
                                    (nome_norm, unid_norm),
                                    [],
                                ).append(obra)
                                por_nome.setdefault(nome_norm, []).append(obra)
                        ...
                        for card in cards_execucao:
                            ...
                            if not card_id or not nome_card:
                                continue

                            unidade_card = _identificar_unidade_card_trello(card)
                            nome_norm = normalizar(nome_card)
                            unid_norm = normalizar(unidade_card)

                            obra_match = por_card_id.get(card_id)

                            # Migra obras antigas, criadas antes de armazenarmos
                            # trello_card_id. Primeiro exige nome + unidade.
                            if not obra_match:
                                candidatos = por_nome_unidade.get(
                                    (nome_norm, unid_norm),
                                    [],
                                )
                                if len(candidatos) == 1:
                            ...
                            if obra_match:
                                ...
                                mudou = (
                                    str(obra_match.get("nome") or "").strip()
                                    != nome_card
                                    or normalizar(
                                        obra_match.get("unidade") or ""
                                    ) != unid_norm
                                    or str(
                                        obra_match.get("trello_card_id") or ""
                                    ).strip() != card_id
                                    or str(
                                        obra_match.get("trello_list_id") or ""
                                    ).strip() != list_id
                                )
                                ...
                                por_card_id[card_id] = obra_match
                            ...
                            try:
                                ...
                                nova_row = cur.fetchone()
                                novo_id = (
                                    nova_row.get("id")
                                    if isinstance(nova_row, dict)
                                    else nova_row[0]
                                )
                ...
                # Backend legado: mantém compatibilidade. A totalidade ainda é
                # percorrida, mas sem as colunas de identidade do Trello.
                ...
                for card in cards_execucao:
                    ...
                    unidade_card = _identificar_unidade_card_trello(card)
                    chave = (
                        normalizar(nome_card),
                        normalizar(unidade_card),
                    )
                    if chave in nomes_unidades:
                        ja_existentes += 1
                        continue
            ...
            _registrar_estado_sync_trello(
                sucesso=False,
                total_cards=total_cards_execucao,
                inseridas=novas_inseridas,
                atualizadas=atualizadas,
                existentes=ja_existentes,
                erro=f"{type(e).__name__}: {str(e)[:300]}",
            )
            return False, (
                "Não foi possível gravar todas as obras de EM EXECUÇÃO. "
                f"Detalhe: {type(e).__name__}: {str(e)[:180]}"
            )
        ...
        if falhas:
            ...
            return False, (
                f"A lista {nome_alvo} possui {total_cards_execucao} card(s), "
                f"mas {len(falhas)} não puderam ser confirmados no banco. "
                "A sincronização será tentada novamente."
            )

        limpar_cache_operacional()
        try:
            _estado_sync_trello_obras.clear()
        except Exception:
            pass
        ...
        return True, (
            f"Sincronização de {nome_alvo} concluída com "
            f"{total_cards_execucao} card(s): "
            f"{novas_inseridas} nova(s), "
            f"{atualizadas} atualizada(s) e "
            f"{ja_existentes} já correta(s)."
        )
...
def sincronizar_trello_automatico_12h():
    """
    Sincronização automática oficial.

    - intervalo de sucesso: 12 horas;
    - se a tentativa falhar, não martela o Trello: espera 30 minutos;
    - o estado fica no Neon, então não depende da sessão/navegador.
    """
    if DB_BACKEND != "NEON" or not hasattr(supabase, "_connect"):
        return False, "automático indisponível sem Neon"

    ultimo_sucesso, ultima_tentativa = _estado_sync_trello_obras()
    agora = agora_aproar()

    if ultimo_sucesso is not None:
        try:
            delta = agora - ultimo_sucesso
            if delta.total_seconds() < TRELLO_AUTO_SYNC_INTERVALO_SEGUNDOS:
                return True, "sincronização automática ainda dentro das 12h"
        except Exception:
            pass

    if ultima_tentativa is not None:
        try:
            delta_tentativa = agora - ultima_tentativa
            if delta_tentativa.total_seconds() < TRELLO_AUTO_SYNC_RETRY_SEGUNDOS:
                return False, "aguardando janela de nova tentativa"
        except Exception:
            pass

    # Registra a tentativa antes do acesso externo para impedir várias
    # sessões simultâneas de dispararem o Trello ao mesmo tempo.
    _registrar_estado_sync_trello(
        sucesso=False,
        erro="sincronização automática iniciada",
    )
    try:
        _estado_sync_trello_obras.clear()
    except Exception:
    ...
    return executar_sincronizacao_trello(
...
# Primeiro acesso ao app depois de 12h também sincroniza, como contingência
# ao agendamento externo.
try:
    _trello_auto_ok, _trello_auto_msg = sincronizar_trello_automatico_12h()
except Exception as _trello_auto_exc:
    _trello_auto_ok = False
    _trello_auto_msg = (
        f"{type(_trello_auto_exc).__name__}: "
        f"{str(_trello_auto_exc)[:180]}"
    )
...
def render_dashboard_consulta(key_prefix="dash", engenheiro_fixo=None):
    ...
    st.markdown(
        ...
        <div class="aproar-dash-metrics">
            ...
            <div class="aproar-dash-card"><div class="aproar-dash-label">Custo total</div><div class="aproar-dash-value" style="font-size:24px">{formatar_reais(custo)}</div><div class="aproar-dash-note">Adic. noturno: {formatar_reais(total_adicional_noturno)}</div></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    cols = [
        ...
        "Custo c/ encargos (R$)", "Adicional noturno (R$)",
    ...
    for conv in render:
        ...
        with st.container(border=True):
            ...
            with st.container():
                ...
                diaria_atual = valor_diaria_financeiro_registro(
                    conv,
                    colab,
                )
                ...
                if eh_sebrae_card:
                    p1, p2, p4, p5 = st.columns([1, 1.25, 1, 1.2])
                else:
                    p1, p2, p5 = st.columns([1, 1.25, 1.2])
                    p4 = None
                ...
                with p2:
                    valor_diaria_financeiro = st.number_input(
                        ...
                        help=(
                            ...
                            "Informe o valor realmente combinado para este dia; "
                            "ele não altera o custo da Controladoria."
                        ),
                    )
...
"
            ...
            with st.form(
                ...
                for _idx_form, conv in enumerate(render_campo, start=1):
                    ...
                    with st.container(
                        ...
                        diaria_atual_pag = valor_diaria_financeiro_registro(
                            conv,
                            colab,
                        )
    ...
    tab_fin_pag, tab_fin_aus, tab_fin_rel = st.tabs([
    ...
    with tab_fin_pag:
        ...
        if resumo_fin.empty:
            st.info(
                "Nenhum valor para pagamento foi lançado neste ciclo."
            )
        else:
            resumo_view = resumo_fin.copy()
            for _c in ["Financeiro (R$)", "Adic. noturno (R$)", "Acordos / Bonificações (R$)", "Total a Pagar (R$)"]:
    ...
    # --- 7. CONFIGURAÇÕES E SINCRONIZAÇÃO TRELLO ---
        ...
        with tab_cad_colab:
            ...
            if ativos_admin:
                ...
                mapa_edicao_id = {
                    str(c.get("id")): c
                    for c in colaboradores_edicao
                    if c.get("id") is not None
                }

                colaborador_id_edicao = st.selectbox(
                    "Funcionário",
                    list(mapa_edicao_id.keys()),
                    format_func=lambda cid: (
                        f"{mapa_edicao_id[cid].get('nome')} · "
                        f"{mapa_edicao_id[cid].get('funcao') or '-'}"
                    ),
                    key="sel_editar_funcionario_admin_id",
                )

                colab_edicao = mapa_edicao_id[
                    str(colaborador_id_edicao)
                ]
                ...
                with st.form(
                    ...
                    nome_edit = st.text_input(
                        ...
                        key=f"edit_nome_func_{colab_edicao.get('id')}",
                    )
                    ...
                    funcao_edit = st.text_input(
                        ...
                        key=f"edit_funcao_func_{colab_edicao.get('id')}",
                    )
                    ...
                    with ce1:
                        valor_edit = st.number_input(
                            ...
                            step=0.01,
                            format="%.2f",
                            key=f"edit_valor_func_{colab_edicao.get('id')}",
                        )
                    ...
                    with ce2:
                        moradia_edit = st.selectbox(
                            ...
                            MORADIAS_COLAB,
                            index=idx_moradia,
                            key=f"edit_moradia_func_{colab_edicao.get('id')}",
                        )
                    ...
                    if salvar_edicao:
                        ...
                        if not nome_editado:
                            ...
                            try:
                                ...
                                for _k in (
                                    f"edit_nome_func_{colab_edicao.get('id')}",
                                    f"edit_funcao_func_{colab_edicao.get('id')}",
                                    f"edit_valor_func_{colab_edicao.get('id')}",
                                    f"edit_moradia_func_{colab_edicao.get('id')}",
                                ):
                                ...
                                st.session_state[
                                    ...
                                    f"Cadastro de {nome_editado} atualizado com sucesso."
                                )
                                st.rerun()
